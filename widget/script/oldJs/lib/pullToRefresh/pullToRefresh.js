...
(function(window) {
    ...
    pullToRefresh.prototype = {
        ...
        _init : function(callback) {
            var self = this;
            var loadingHtml = '<div class="refresh-load"><div class="refresh-pull-arrow"></div></div>';
        ...
        touchMove : function (ev) {
            ...
            var moving = function() {
                ...
                if(touchYDelta > self.params.triggerDistance){
                    self.params.container.classList.add("refresh-pull-up");
                    self.params.container.classList.remove("refresh-pull-down");
                }else{
                    self.params.container.classList.add("fefresh-pull-down");
                    self.params.container.classList.remove("refresh-pull-up");
                }
            };
        ...
        touchEnd : function (ev,callback) {
            ...
            if( touchYDelta >= this.params.triggerDistance) {
                ...
                document.querySelector(".refresh-pull-arrow").style.webkitTransitionDuration =
                    document.querySelector(".refresh-pull-arrow").style.transitionDuration = '0ms';
                self.params.container.classList.add("refreshing");
                if(callback){
                    callback({
                        status:"success"
                    });
        ...
        cancelLoading : function () {
            var self =this;
            isLoading = false;
            self.params.container.classList.remove("refreshing");
            document.querySelector(".refresh-pull-arrow").style.webkitTransitionDuration =
                document.querySelector(".refresh-pull-arrow").style.transitionDuration = '300ms';
            this.params.container.style.webkitTransitionDuration =
                this.params.container.style.transitionDuration = '300ms';
            self.params.container.style.webkitTransform =
                self.params.container.style.transform = 'translate3d(0,0,0)';
            self.params.container.classList.remove("refresh-pull-up");
            self.params.container.classList.add("refresh-pull-down");
            return;
        },
        scrollY : function() {
